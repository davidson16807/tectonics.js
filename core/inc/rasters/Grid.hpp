...
public:
		  ...
		  face_vertex_ids(faces)
	{
		...
		arrow_vertex_ids = uvec2s(arrow_vertex_ids_set.begin(), arrow_vertex_ids_set.end());
		...
		{
			if (arrow_vertex_ids[i].y > arrow_vertex_ids[i].x)
			{
				edge_vertex_ids_vector.push_back(uvec2(arrow_vertex_ids[i]));
			}
		}
