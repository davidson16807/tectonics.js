...
function RealisticWorldView(shader_return_value) {
    ...
    function update_renderpass_vertex_shader(material, value) {
    ...
    function update_renderpass_uniform(material, key, value) {
    ...
    function update_shaderpass_uniform(material, key, value) {
    ...
    function update_renderpass_attribute(geometry, key, raster) {
    ...
    this.updateScene = function(gl_state, world, options) {
        ...
        update_renderpass_vertex_shader(mesh.material, options.vertexShader);
        update_renderpass_uniform  (mesh.material, 'projection_matrix_inverse',projection_matrix_inverse);
        update_renderpass_uniform  (mesh.material, 'view_matrix_inverse',  gl_state.camera.matrixWorld);
        update_renderpass_uniform  (mesh.material, 'reference_distance',   world.radius);

        update_renderpass_uniform  (mesh.material, 'light_rgb_intensity',  light_rgb_intensity_threejs);
        update_renderpass_uniform  (mesh.material, 'light_direction',      new THREE.Vector3(1,0,0));
        update_renderpass_uniform  (mesh.material, 'insolation_max',       insolation_max);
        
        update_renderpass_uniform  (mesh.material, 'sealevel_mod',         options.sealevel_mod);
        update_renderpass_uniform  (mesh.material, 'darkness_mod',         options.darkness_mod);
        update_renderpass_uniform  (mesh.material, 'ice_mod',              options.ice_mod);
        update_renderpass_uniform  (mesh.material, 'index',                options.index);
        update_renderpass_uniform  (mesh.material, 'world_radius',         world.radius);
        update_renderpass_uniform  (mesh.material, 'sealevel',             world.hydrosphere.sealevel.value());

        update_renderpass_attribute(mesh.geometry, 'displacement',         world.lithosphere.displacement.value());
        update_renderpass_attribute(mesh.geometry, 'ice_coverage',         world.hydrosphere.ice_coverage.value());
        update_renderpass_attribute(mesh.geometry, 'surface_temp',         world.atmosphere.surface_temp);
        update_renderpass_attribute(mesh.geometry, 'plant_coverage',       world.biosphere.plant_coverage.value());



        // SHADERPASS PROPERTIES -----------------------------------------------

        update_shaderpass_uniform  (shaderpass,    'projection_matrix_inverse',projection_matrix_inverse);
        update_shaderpass_uniform  (shaderpass,    'view_matrix_inverse',      gl_state.camera.matrixWorld);
        update_shaderpass_uniform  (shaderpass,    'reference_distance',       world.radius);

        update_shaderpass_uniform  (shaderpass,    'light_rgb_intensity',      light_rgb_intensity_threejs);
        update_shaderpass_uniform  (shaderpass,    'light_direction',          new THREE.Vector3(1,0,0));
        update_shaderpass_uniform  (shaderpass,    'insolation_max',           insolation_max);

        update_shaderpass_uniform  (shaderpass,    'world_position',           new THREE.Vector3());
        update_shaderpass_uniform  (shaderpass,    'world_radius',             world.radius);
        ...
        update_shaderpass_uniform  (shaderpass,    'atmosphere_scale_height', 
        ...
        update_shaderpass_uniform  (shaderpass, 'atmosphere_surface_rayleigh_scattering_coefficients', new THREE.Vector3(5.20e-6, 1.21e-5, 2.96e-5));
        update_shaderpass_uniform  (shaderpass, 'atmosphere_surface_mie_scattering_coefficients',      new THREE.Vector3(2.1e-9,  2.1e-9,  2.1e-9 ));
        update_shaderpass_uniform  (shaderpass, 'atmosphere_surface_absorption_coefficients',          new THREE.Vector3(0));
    };
