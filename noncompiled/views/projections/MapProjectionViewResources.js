...
*/
function MapProjectionViewResources(initial_view_state) {
    ...
    function get_options(view_state, offset) {
        return Object.assign({
                    world_view_vertex_shader: view_state.vertex_shader, 
                    shaderpass_visibility: 0,
                    map_projection_offset: offset * 0.75 * Math.PI,
                }, view_state);
    }
    ...
    let subview1 = new get_ResourceClass(initial_view_state)(get_options(initial_view_state,  1));
    let subview2 = new get_ResourceClass(initial_view_state)(get_options(initial_view_state, -1));
    ...
    */
    this.addToScene = function(gl_state) {
        subview1.addToScene(gl_state);
        subview2.addToScene(gl_state);
    };
    /*
    ...
    */
    this.updateScene = function(gl_state, model, view_state) {
        if(subview_type !== view_state.subview_type || 
           subview1.needsRemoval(model, view_state)) {
            subview_type = view_state.subview_type;
            this.removeFromScene(gl_state);
            subview1 = new get_ResourceClass(view_state)(get_options(view_state,  1));
            subview2 = new get_ResourceClass(view_state)(get_options(view_state, -1));
            this.addToScene(gl_state);
        }
        subview1.updateScene(gl_state, model, get_options(view_state,  1));
        subview2.updateScene(gl_state, model, get_options(view_state, -1));
    };
    /*
...
window.VIEW_RESOURCES = window.VIEW_RESOURCES || {};
window.VIEW_RESOURCES['MapProjectionViewResources'] = MapProjectionViewResources;