...
const Tectonophysics = (function() {

const Tectonophysics = {};
...
Tectonophysics.guess_plate_velocity = function(boundary_normal, buoyancy, material_viscosity, result) {
    ...
    const width = 300e3; // meters 
    const length = 600e3; // meters
    const thickness = 100e3; // meters
    const effective_area = Math.pow(thickness * length * width, 2/3); // m^2
    const shape_parameter = 0.725; // unitless
    const slab_dip_angle_constant = 4.025; // unitless
    const world_radius = 6367e3; // meters

    const lateral_speed = scratchpad.getFloat32Raster(grid);                 
    let lateral_speed_per_force  = 1;
...
Tectonophysics.get_plate_center_of_mass = function(mass, plate_mask, scratch) {
    ...
    const plate_mass = scratch;
    ScalarField.mult_field             (mass, plate_mask,                     plate_mass);
    const center_of_plate = VectorDataset.weighted_average (plate_mass.grid.pos, plate_mass);
...
Tectonophysics.get_plate_rotation_matrix3x3 = function(plate_velocity, center_of_plate, seconds) {
    ...
    const grid = plate_velocity.grid;
    ...
    const center_of_world_offset = grid.pos;
    const center_of_plate_offset = scratchpad.getVectorRaster(grid);
    const center_of_plate_distance2 = scratchpad.getFloat32Raster(grid);
//    const center_of_world_distance2 = scratchpad.getFloat32Raster(grid); // NOTE: center_of_world_distance2 is not effectively used
...
//    VectorField.fill                 (center_of_world_distance2, 1); // NOTE: center_of_world_distance2 is not effectively used
    ...
    const center_of_plate_angular_velocity = scratchpad.getVectorRaster(grid);
    ...
    const center_of_world_angular_velocity = scratchpad.getVectorRaster(grid);
...
//    VectorField.div_scalar_field     (center_of_world_angular_velocity, center_of_world_distance2, center_of_world_angular_velocity); // NOTE: equivalent to division by 1
    ...
    const plate_velocity_magnitude = scratchpad.getFloat32Raster(grid);
    VectorField.magnitude             (plate_velocity,                     plate_velocity_magnitude);
    const is_pulled = scratchpad.getUint8Raster(grid);
    ScalarField.gt_scalar             (plate_velocity_magnitude, 3e-18,    is_pulled);
// console.log(Uint8Dataset.sum(is_pulled))

    const center_of_plate_angular_velocity_average = VectorDataset.weighted_average(center_of_plate_angular_velocity, is_pulled);
    const center_of_world_angular_velocity_average = VectorDataset.weighted_average(center_of_world_angular_velocity, is_pulled);

    const center_of_plate_rotation_vector = Vector.mult_scalar(center_of_plate_angular_velocity_average.x, center_of_plate_angular_velocity_average.y, center_of_plate_angular_velocity_average.z, seconds);
    const center_of_world_rotation_vector = Vector.mult_scalar(center_of_world_angular_velocity_average.x, center_of_world_angular_velocity_average.y, center_of_world_angular_velocity_average.z, seconds);

    // TODO: negation shouldn't theoretically be needed! find out where the discrepancy lies and fix it the proper way! 
    let center_of_plate_rotation_matrix = Matrix3x3.FromRotationVector(-center_of_plate_rotation_vector.x, -center_of_plate_rotation_vector.y, -center_of_plate_rotation_vector.z);
    if (isNaN(center_of_plate_rotation_matrix[0])) {
        center_of_plate_rotation_matrix = Matrix3x3.Identity();
    }
    let center_of_world_rotation_matrix = Matrix3x3.FromRotationVector(-center_of_world_rotation_vector.x, -center_of_world_rotation_vector.y, -center_of_world_rotation_vector.z);
    if (isNaN(center_of_world_rotation_matrix[0])) {
        center_of_world_rotation_matrix = Matrix3x3.Identity();
    }

    const rotation_matrix = Matrix3x3.mult_matrix(center_of_plate_rotation_matrix, center_of_world_rotation_matrix);
...
Tectonophysics.guess_plate_map = function(vector_field, segment_num, min_segment_size, segments) {
  segments = segments || Uint8Raster(vector_field.grid);
  ...
  const dilation    = BinaryMorphology.dilation;
  const closing     = BinaryMorphology.closing;
  const difference  = BinaryMorphology.difference;
  const fill_ui8    = Uint8RasterGraphics.fill_into_selection;

  // step 2: dilate and take difference with (segments != i)
  const segment = Uint8Raster(vector_field.grid);
  const is_empty = Uint8Raster(vector_field.grid);
  const is_occupied = Uint8Raster(vector_field.grid);
  const plate_ids = Uint8Dataset.unique(segments);
  let plate_id = 0;
