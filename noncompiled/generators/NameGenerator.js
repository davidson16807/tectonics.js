...
const NameGenerator = (function() {
    ...
    get_markov_model = function(corpus) {
        const bigrams = get_bigrams(corpus);
        const markov_model = {};
        for (let prev in bigrams){
    ...
    get_near_length_cost_fn = function(length) {
        return function(text) {
            const distance = text.length - length;
    ...
    get_generator = function(corpus, tries, cost_fn) {
        const markov_model = get_markov_model(corpus);
        return function(random) {
    ...
    get_length_distribution_generator = function(corpus, tries, length_distribution) {
        const get_random_length = random_weighted_choice(length_distribution);
        const markov_model = get_markov_model(corpus);
        return function(random) {
            const length = parseInt(get_random_length(random));
    ...
    get_realistic_length_generator = function(corpus, tries) {
        const length_distribution = get_distribution(corpus.map(text=>text.length));
