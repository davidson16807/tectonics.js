...
var NameGenerator = (function() {
	...
	run_markov_model = function(markov_model) {
		var prev=' '
		var next=''
		var text=''
		for (var i = 0; i < 20 && next != ' '; i++) {
			next = markov_model[prev]();
			text += next;
			prev = next;
		}
		return text.trim()	
	...
	run_markov_model_best_of = function(markov_model, tries, cost_fn) {
		var text='';
		var cost=0.0;
		var best_text='';
		...
		for (var i = 0; 
			 i < tries; i++) {
			text = run_markov_model(markov_model);
			cost = cost_fn(text);
	...
	get_realistic_length_generator = function(corpus, tries) {
		...
		return get_length_distribution_generator(corpus, tries, length_distribution);
	}
	...
	test= function(corpus, tries, distribution, num_generated) {
		gen = get_length_distribution_generator(corpus, tries, distribution);
		generated = [];
