...
FUNC(float) get_fraction_of_light_reflected_on_surface(IN(float) cos_incident_angle, IN(float) characteristic_reflectance)
{
...
//   see Hoffmann 2015 for a gentle introduction to the concept
//   see Schlick (1994) for implementation details
FUNC(vec3) get_rgb_fraction_of_light_reflected_on_surface(IN(float) cos_incident_angle, IN(vec3) characteristic_reflectance)
{
...
FUNC(float) get_fraction_of_reflected_light_masked_or_shaded(IN(float) cos_view_angle, IN(float) root_mean_slope_squared)
{
...
//   see Hoffmann 2015 for a gentle introduction to the concept.
//   see Schlick (1994) for even more details.
FUNC(float) get_fraction_of_microfacets_with_angle(IN(float) cos_angle_of_deviation, IN(float) root_mean_slope_squared)
{
    float m = root_mean_slope_squared;
